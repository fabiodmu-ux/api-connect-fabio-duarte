...
## 🎯 Objetivo da API
Esta é uma API REST de gerenciamento de usuários desenvolvida como solução técnica para fins didáticos e avaliativos. O objetivo do projeto é consolidar conceitos fundamentais de desenvolvimento web, manipulação de protocolos e métodos HTTP (GET, POST, PUT, DELETE), validação de payloads com JSON e tratamento de códigos de status (Status Codes).

A aplicação utiliza **persistência de dados baseada em arquivo JSON**, garantindo que as operações de criação, atualização e deleção reflitam em um armazenamento confiável, evitando colisões de IDs.
...
```bash
git clone https://github.com/[SEU_USUARIO_DO_GITHUB]/api-connect-fabio-duarte.git
```

### 3. Entrar na Pasta do Projeto:
```bash
cd api-connect-fabio-duarte
```
...
0. Health Check (Status da API)
Rota: /api/status

#### Método: GET

Descrição: Verifica a disponibilidade e a saúde da API.
